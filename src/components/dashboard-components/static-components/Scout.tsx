import { SCOUT_INSIGHTS } from "../../constants";
import SoutImage from "../../../assets/images/scout.png";
export const ScoutData = () => {
  return (
    <div className="flex gap-4 w-full  p-8 flex-col">
      <div className="flex flex-col gap-2">
        <img
          src="https://i.ibb.co/k212gGd1/Screenshot-2025-12-12-at-12-23-54-PM.png"
          alt=""
        />
        <img
          src="https://i.ibb.co/dwt0byq0/Screenshot-2025-12-12-at-12-28-26-PM.png"
          alt=""
        />
        <img
          src="https://i.ibb.co/1YHrPczN/Screenshot-2025-12-12-at-12-29-03-PM.png"
          alt=""
        />
      </div>
      <div className="flex gap-4 w-full border rounded-md p-8">
        {SCOUT_INSIGHTS.map((item) => (
          <div
            key={item.id}
            className="flex flex-col gap-4 bg-white w-1/4 p-4 rounded-lg justify-between"
          >
            <div className="flex gap-2">
              {item.badges.map((item, i) => (
                <div
                  className="border p-1 rounded-lg text-xs bg-green-100 text-green-800 font-bold"
                  key={i}
                >
                  {item}
                </div>
              ))}
            </div>
            <div className="font-bold flex gap-2">
              <svg
                viewBox="0, 0, 576, 512"
                focusable="false"
                className="h-6 w-6"
                aria-hidden="true"
              >
                <path
                  fill="currentColor"
                  d="M327.8 224c5.3-15 8.2-31.1 8.2-48c0-79.5-64.5-144-144-144S48 96.5 48 176c0 30.6 9.5 58.8 25.7 82.1c4.1 5.9 8.8 12.3 13.6 19c0 0 0 0 0 0c12.7 17.5 27.1 37.2 38 57.1c8.9 16.2 13.7 33.3 16.2 49.9L109 384c-2.2-12-5.9-23.7-11.8-34.5c-9.9-18-22.2-34.9-34.5-51.8c0 0 0 0 0 0s0 0 0 0s0 0 0 0c-5.2-7.1-10.4-14.2-15.4-21.4C27.6 247.9 16 213.3 16 176C16 78.8 94.8 0 192 0s176 78.8 176 176c0 16.6-2.3 32.7-6.6 48l-33.6 0zM256 416l0 2c0 14.2 4.6 27.2 12.4 37.8C258.2 488.4 227.9 512 192 512c-44.2 0-80-35.8-80-80l0-5.3c0-5.9 4.8-10.7 10.7-10.7l69.3 0 64 0zM112 176c0 8.8-7.2 16-16 16s-16-7.2-16-16c0-61.9 50.1-112 112-112c8.8 0 16 7.2 16 16s-7.2 16-16 16c-44.2 0-80 35.8-80 80zm80 304c20.9 0 38.7-13.4 45.3-32l-90.5 0c6.6 18.6 24.4 32 45.3 32zM320 256l224 0c17.7 0 32 14.3 32 32l0 128c0 17.7-14.3 32-32 32l-64 0 0 48c0 6.5-3.9 12.3-9.9 14.8s-12.9 1.1-17.4-3.5L393.4 448 320 448c-17.7 0-32-14.3-32-32l0-128c0-17.7 14.3-32 32-32zm0 32l0 128 80 0c4.2 0 8.3 1.7 11.3 4.7L448 457.4l0-25.4c0-8.8 7.2-16 16-16l80 0 0-128-224 0z"
                ></path>
              </svg>
              <div>{item.title}</div>
            </div>
            <div className="line-clamp-3">{item.desc}</div>
            <a
              href={`${item.link}`}
              className="text-[#0176a0] hover:underline flex gap-2 items-center"
              target="_blank"
            >
              <p>Explore This Insight</p>

              <svg
                viewBox="0, 0, 448, 512"
                focusable="false"
                className="h-5 w-5 text-[#0176a0]"
                aria-hidden="true"
              >
                <path
                  fill="currentColor"
                  d="M443.3 267.3c6.2-6.2 6.2-16.4 0-22.6l-176-176c-6.2-6.2-16.4-6.2-22.6 0s-6.2 16.4 0 22.6L393.4 240 16 240c-8.8 0-16 7.2-16 16s7.2 16 16 16l377.4 0L244.7 420.7c-6.2 6.2-6.2 16.4 0 22.6s16.4 6.2 22.6 0l176-176z"
                ></path>
              </svg>
            </a>
          </div>
        ))}
      </div>
      <div className="w-full text-right flex justify-end">
        <a
          style={{ background: "#012f87" }}
          className="text-sm text-right px-4 py-2 w-fit font-bold text-white rounded-lg flex justify-end"
          href="https://www.yext.com/s/514581/scout/scan/01999734-27da-76bf-b464-8d86806b4904/detai[…]lectedLocationId=01999734-278b-70d2-a14f-f75de6f2a332&b=514581"
        >
          View Full report {"->"}
        </a>
      </div>
      <img
        className="w-full border py-2 rounded-md"
        src="https://i.ibb.co/zjRj9fF/Screenshot-2025-10-09-at-3-58-01-PM.png"
        alt=""
      />
    </div>
  );
};
